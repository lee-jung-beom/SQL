2021-0406-01)커서(CURSOR)
 - 커서는 쿼리문의 영향을 받은 행들의 집합
 - 묵시적 커서(IMPLICITE), 명시적 (EXPLICITE) 커서로 구분
 - 커서의 선언은 선언부에서 수행
...
1)묵시적 커서
 ...
 (1)생성
  (사용형식)
   ...
   IS 
    SELECT 문;
   사용예) 상품매입테이블(BUYPROD)에서 2005년 2월 상품별 매입현황(상품코드, 상품명), 거래처별, 매입수량)
